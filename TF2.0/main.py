...
def one_hot_econding(vect, N_classes, N_ch):
    ...
    dimension - size(vect, 1) x N_class

    """
    one_hot_matrix = tf.one_hot(vect, N_classes)
...
def initialize_parameters(layers, activation, stbeta):
    ...
    tf.random.set_seed(1)                   # defines the seed of the random number generator
    ...
    for l in range(1, L):
        parameters['w' + str(l)] = tf.Variable( tf.zeros([layers[l], layers[l-1]]), True, name = 'w' + str(l))
        parameters['b' + str(l)] = tf.Variable( tf.zeros([layers[l], 1]), True, name ='b' + str(l))
        parameters['beta' + str(l)] = tf.Variable( tf.zeros([layers[l], 1]), train, name = 'beta'+ str(l))
...
def Run_DNN(X_train, Y_train, X_test, Y_test, layers, activation, epoch_sample, stdbeta, starter_learning, num_iterations, with_hessian, save_model, Plot):
    ...
    rate = tf.compat.v1.train.exponential_decay(starter_learning, global_step, 500, 0.9) #exponential learning rate decay
    #rate = starter_learning

    tvars = parameters.trainable_variables #list of trainable variables
    ...
    for epoch in range(num_iterations):
      ...
      epoch_cost = obj([an,hn], betan, Y, activation)

      epoch_grad = optimizer.compute_gradients(epoch_cost, tvars) #Get gradients layer by layer. Note that this function returns the pair (grads, var)
      grads = [epoch_grad[i][0] for i in range(len(epoch_grad))] #extract the gradients
      epoch_acc_train = eval_accuracy(Y, an, actL)

      min   = optimizer.apply_gradients(grads_and_vars= epoch_grad, global_step= global_step)    #Apply the gradients to look for critical points
    ...
    print('Train accuracy', acc_train[num_iteratons-1])
    # print('Test accuracy', acc_test[num_iterations-1])
